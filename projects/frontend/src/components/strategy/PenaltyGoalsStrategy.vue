...
const dummyModel = ref<{ kind: "penalty_goals", "entry": { country_a_goals: number, country_b_goals: number } }>({ kind: 'penalty_goals', entry: { country_b_goals: 0, country_a_goals: 0 } });
const scoreEntered = ref(false);

if (hasCompleted || props.forResult) {
  const resultEntry = props.tipStrategy.result?.strategyResults?.find((entry) => entry.kind === 'penalty_goals')
  if (resultEntry) {
...
if (hasCompleted && props.tipStrategy.tips) {
  const goals = props.tipStrategy.tips[0]?.strategies.find((entry) => entry.kind == 'penalty_goals');
  if (goals) {
