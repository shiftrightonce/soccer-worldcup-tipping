<template>
  ...
  <SelectCountries v-if="!hasCompleted && countries.length" v-model="model" :countries="countries"
    :tip-strategy="props.tipStrategy" :max="1" :is-closed="props.isClosed" :for-result="props.forResult"
    label="First to get a yellow card">
  </SelectCountries>
</template>
...
onMounted(() => {
  ...
  if (hasCompleted && props.tipStrategy.tips) {
    const firstRedCard = props.tipStrategy.tips[0]?.strategies.find((entry) => entry.kind == 'first_yellow_card');
    if (firstRedCard) {
