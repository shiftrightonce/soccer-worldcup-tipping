    ...
    <q-tab-panels keep-alive v-model="completedViewTabs" animated>
      <q-tab-panel name="result">
        ...
        <SelectCountries v-if="scoreEntered && model.entry && countries.length" v-model="model" :countries="countries"
          :tip-strategy="props.tipStrategy" :is-closed="hasCompleted" :max="32" label="">
        </SelectCountries>
      ...
      <q-tab-panel name="yours">
        ...
        <SelectCountries v-if="userEntered && dummyModel.entry && countries.length" v-model="dummyModel"
          :countries="countries" :tip-strategy="props.tipStrategy" :is-closed="hasCompleted" :max="32" label="">
        </SelectCountries>
      </q-tab-panel>

    </q-tab-panels>
  </div>
  <SelectCountries v-if="isReady && !hasCompleted" :tipStrategy="props.tipStrategy" :max="32" :countries="countries"
    :is-closed="props.isClosed" label="Select 32 countries" :for-result="props.forResult" v-model="model">
  </SelectCountries>
</template>
...
const dummyModel = ref<{ kind: "round_32_qualifiers", "entry": string[] }>({ kind: 'round_32_qualifiers', entry: [] });
...
onMounted(async () => {
  ...
  if (hasCompleted && props.tipStrategy.tips) {
    const userEntry = props.tipStrategy.tips[0]?.strategies.find((entry) => entry.kind == 'round_32_qualifiers');
    if (userEntry) {
